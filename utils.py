...
def to_cuda(data):
    if len(data) == 14:
        ...
        ser_mels = torch.from_numpy(ser_mels).float().cuda()
        ...
        speaker_ids = torch.from_numpy(speaker_ids).cuda()
        emotion_labels = torch.from_numpy(emotion_labels).cuda()
...
def to_device(data, device):
    if len(data) == 14:
        ...
        ser_mels = torch.from_numpy(ser_mels).float().to(device)
        mel_lens = torch.from_numpy(mel_lens).to(device)
        pitches = torch.from_numpy(pitches).float().to(device)
        energies = torch.from_numpy(energies).to(device)
        durations = torch.from_numpy(durations).long().to(device)
        speaker_ids = torch.from_numpy(speaker_ids).to(device)
        emotion_labels = torch.from_numpy(emotion_labels).to(device)
    ...
    raise Exception(f"Unknown data length, {len(data)}")
