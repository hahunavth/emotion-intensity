...
class StrengthNet(nn.Module):
    ...
    def __init__(self, n_classes=4):
        ...
        self.emotion_embedding = nn.Embedding(5, 128)
        ...
        self.attn = Attention(128, batch_first=True)
    ...
    def forward(self, x, x_lens, emo_ids):
        """
        ...
        emo_embed = self.emotion_embedding(emo_ids)
        # expand and padding to match x
        # emo_embed = emo_embed.unsqueeze(1).repeat(1, x.size(1), 1)
        # emo_embed = F.pad(input=emo_embed, pad=(512-16, 0), mode='constant', value=0)
        # x = x + emo_embed

        x = nn.utils.rnn.pack_padded_sequence(x, x_lens, batch_first=True, enforce_sorted=False)
        # BLSTM1
        h = torch.stack([emo_embed, emo_embed])
        c = torch.zeros(2, 32, 128, device=device)
        out, (hn, cn) = self.blstm1(x, (h, c))
        ...
        attn_out, attns = self.attn(out, x_lens)
        if attn_out.ndim == 1:
            attn_out = attn_out.unsqueeze(0)
        emo_pred = self.fc2(attn_out)

        # Frame Score
        frame_score = self.frame_fc(attn_out)
...
#     p_hat_ij = F.sigmoid(ri - rj)
#     rank_loss = torch.sum(
#         # + lam_diff * lam_diff *  -torch.log2(p_hat_ij) + (1 - lam_diff) * 1 - lam_diff *  - torch.log2(1 - p_hat_ij)
#         + lam_diff * torch.sqrt(lam_diff) *  -torch.log2(p_hat_ij) + (1 - lam_diff) * torch.sqrt(1 - lam_diff) *  - torch.log2(1 - p_hat_ij)
#     ) / lam_diff.size(0)
#     return rank_loss
...
if __name__ == "__main__":
    ...
    mix_train_ds = RankMixupDataset(train_ds, emo_ids, neu_ids, alpha=1.)
    ...
    for epoch in range(50):
        ...
        for idx, batch in enumerate(train_loader):
            ...
            ri, _, hi = model(xi, xi_lens, y_emo)
            rj, _, hj = model(xj, xj_lens, y_emo)
        ...
        for idx, batch in enumerate(val_loader):
            ...
            rank_true.append((F.sigmoid(ri) < F.sigmoid(rj) - 0.1).detach().cpu())
